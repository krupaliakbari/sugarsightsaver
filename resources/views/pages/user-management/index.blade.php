                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                            ...
                            {{-- <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#approvalModal">
                                <i class="fas fa-check-circle me-2"></i>Bulk Approval
                            </button> --}}
                                </div>
                        </div>
                                    ...
                                    <div class="filter-row">
                                        ...
                                        <select class="form-select" name="doctor_type">
                                            <option value="">All Types</option>
                                                <option value="diabetes_treating" {{ request('doctor_type') == 'diabetes_treating' ? 'selected' : '' }}>Diabetes Treating</option>
                                                <option value="ophthalmologist" {{ request('doctor_type') == 'ophthalmologist' ? 'selected' : '' }}>Ophthalmologist</option>
                                        </select>
                                    </div>
                                        <div class="filter-buttons">
                                            ...
                                            @if(request()->hasAny(['search', 'status', 'approval_status', 'doctor_type']))
                                                <a href="{{ route('user-management') }}" class="btn btn-outline-secondary btn-clear">
        ...
        <thead>
            <tr>
                ...
                <th class="sortable-header" data-sort-column="email">
                    Contact
                    <span class="sort-arrows">
                        <span class="sort-arrow {{ $sortBy === 'email' && $sortDirection === 'asc' ? 'active' : '' }}">▲</span>
                        <span class="sort-arrow {{ $sortBy === 'email' && $sortDirection === 'desc' ? 'active' : '' }}">▼</span>
                    </span>
                </th>
                <th class="sortable-header" data-sort-column="hospital_name">
                    Hospital & Type
                    <span class="sort-arrows">
                        <span class="sort-arrow {{ $sortBy === 'hospital_name' && $sortDirection === 'asc' ? 'active' : '' }}">▲</span>
                        <span class="sort-arrow {{ $sortBy === 'hospital_name' && $sortDirection === 'desc' ? 'active' : '' }}">▼</span>
                    </span>
                </th>
            ...
            <tr>
                    ...
                    <div class="d-flex align-items-center">
                        <div class="avatar avatar-sm me-3" style="width: 40px; height: 40px;">
                            @if($doctor->profile_image)
                                <img src="{{ url($doctor->profile_image) }}" alt="avatar" class="rounded-circle" style="width: 100%; height: 100%; object-fit: cover;">
                            @else
                        ...
                        <div>
                            <h6 class="mb-0">{{ $doctor->name ?? 'No Name' }}</h6>
                            <small class="text-muted">{{ $doctor->qualification ?? 'No qualification' }}</small>
                        </div>
                    </div>
                </td>
                <td>
                    <div>
                        <p class="mb-0">{{ $doctor->email ?? 'No email' }}</p>
                        <small class="text-muted">{{ $doctor->phone ?? 'No phone' }}</small>
                    </div>
                </td>
                <td>
                    <div>
                        <p class="mb-0 text-truncate" style="max-width: 200px;" title="{{ $doctor->hospital_name ?? '' }}">
                            {{ $doctor->hospital_name ?? 'Not specified' }}
                        </p>
                        @if($doctor->doctor_type)
                            <span class="badge badge-light-info badge-sm">
                                {{ $doctor->doctor_type === 'diabetes_treating' ? 'Diabetes Treating' : 'Ophthalmologist' }}
                            </span>
                        @else
                ...
                <td>
                    <div class="d-flex gap-1">
                        <a href="{{ route('user-management.show', $doctor->id) }}" class="btn btn-sm btn-info" title="View Details" style="padding: 0.375rem 0.5rem;">
                        ...
                        <button type="button" class="btn btn-sm btn-warning"
                                                                    title="Edit Doctor"
                                                                    onclick="window.location.href='{{ route('user-management.edit', $doctor->id) }}'"
    ...
    <x-slot:footerFiles>
                ...
                function setupAjaxSorting() {
                    ...
                    sortableHeaders.forEach(function(header) {
                        header.addEventListener('click', function(e) {
                            ...
                            const ajaxUrl = '{{ route("user-management") }}?' + params.toString();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.action = '{{ route("user-management.delete", ":id") }}'.replace(':id', id);
